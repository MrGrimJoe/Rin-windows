...
const char* to_string(AuditLevel level) {
    switch (level) {
        ...
        case AuditLevel::SecuritySuccess: return "SECURITY_SUCCESS";
        case AuditLevel::SecurityWarning: return "SECURITY_WARNING";
        case AuditLevel::SecurityError: return "SECURITY_ERROR";
    }
    return "INFO";
}
...
std::string AuditEvent::formatted_time() const {
    auto time_t_val = static_cast<std::time_t>(timestamp_ms / 1000);
    ...
    std::snprintf(buf, sizeof(buf), "%02d:%02d:%02d.%03d", tm_buf.tm_hour, tm_buf.tm_min,
                  tm_buf.tm_sec, static_cast<int>(ms_part));
...
std::optional<std::string> MeshAuditLogger::sanitize_key(const std::optional<std::string>& key) {
    // Matches MeshAuditLogger.kt's sanitizeKey exactly: never log a full
