...
std::optional<int64_t> TcpTransport::send_packet(const std::string& ip, int port,
    ...
    const char* rail_name = to_wire_string(packet.rail);
    MeshAuditLogger::instance().log_connection_attempt(rail_name, ip, port, packet.sender_name);
    ...
    try {
        ...
        if (connect_ec == asio::error::would_block || connect_ec) {
            MeshAuditLogger::instance().log_connection_failed(
                rail_name, ip, port, connect_ec ? connect_ec.message() : "connect timed out",
                packet.sender_name);
        ...
        if (read_ec || bytes == 0) {
            MeshAuditLogger::instance().log_connection_failed(
                rail_name, ip, port, read_ec ? read_ec.message() : "no ACK received before timeout",
                packet.sender_name);
            return std::nullopt;
        }
        ...
        auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
                           ...
                           .count();
        MeshAuditLogger::instance().log_connection_established(rail_name, ip, port, packet.sender_name);
        return std::max<int64_t>(1, elapsed);
    } catch (const std::exception& e) {
        MeshAuditLogger::instance().log_connection_failed(rail_name, ip, port, e.what(), packet.sender_name);
        return std::nullopt;
    }
}
